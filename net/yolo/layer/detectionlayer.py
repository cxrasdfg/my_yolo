...
class DetectionLayer(torch.nn.Module):
    ...
    def forward(self,*args):
        ...
        if self.training:
            ...
            x_grid=self.x_grid # [side,side]=[7,7]
            y_grid=self.y_grid # [side,side]=[7,7]
            ...
            for idx_batch,(x, # [7,7,35]
                ...
                ccwh_boxes[:,[0,2]]/=(1.*img_w/self.side)
                ccwh_boxes[:,[1,3]]/=(1.*img_h/self.side)
                
                # yolo selects the boxes which gt's center located is in...
                ...
                ccwh_boxes[:,[0,2]]/=1.0*self.side
                ccwh_boxes[:,[1,3]]/=1.0*self.side
                ...
                out_locoff=out_locoff[...,:self.coords] # [side,side,num,coords]
                out_conf=out_locoff[...,self.coords] # [side,side,num]

                out_cls=x[...,self.num*(self.coords+self.rescore):] # [side,side,classes]
                
                # 3. prepare `default box`
                ...
                # 4. from output to predict box...
                # out_loc,out_conf,out_cls
                # I think should add sigmoid on location.
                out_loc=out_loc.sigmoid() # [side,side,num,4]
                ...
                pred_loc[...,:2]=pred_loc[...,:2]+default_xy
                
                # normalize the x,y in pred_loc to [0,1]
                pred_loc[...,:2]=pred_loc[...,:2]/self.side

                # 5. match the box centered in
                ...
                gt_iou_target=gt_loc_target[...,0].float() # [side,side,num]
                gt_cell_box_assign=gt_iou_target.long() # [side,side,num]
                gt_cell_asiign=gt_iou_target[...,0].long() # [side,side]    
                ...
                for gt_loc_xyxy,gt_loc_ccwh,gt_label,cell_i,cell_j in\
                    ...
                    ious=t_box_iou(cell_boxes,gt_loc_xyxy).view(-1) # [num]
                    ...
                    tt[:2]=tt[:2]*self.side-default_xy[cell_i,cell_j,max_box_idx]
                ...
                # 6.prepare the loss function....
                if self.sqrt:
                    gt_loc_target[...,2:]=gt_loc_target[...,2:].sqrt()
                    out_loc[...,2:]=out_loc[...,2:].sqrt()
                
                one_obj_i_j_mask=gt_cell_box_assign>0 # [side,side,num]
                one_obj_i_mask=gt_cell_asiign>0 # [side,side]
                
                loss+=[self.coord_scale*( ((out_loc-gt_loc_target)**2).sum(dim=3) *one_obj_i_j_mask).sum()\
    ...
    def cuda(self,did):
        super(DetectionLayer,self).cuda(did)
        
        if not self.x_grid.is_cuda:
            self.x_grid=self.x_grid.cuda(did)
            self.y_grid=self.y_grid.cuda(did)
        
        return self
    
    def cpu(self):
        super(DetectionLayer,self).cpu()
        
        if self.x_grid.is_cuda:
            self.x_grid=self.x_grid.cpu()
            self.y_grid=self.y_grid.cpu()
